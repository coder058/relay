...
React/TypeScript UI → stateless FastAPI job endpoints → deterministic evidence service and a fixed, bounded public-board reader. The official `mcp==2.1.1` Python SDK exposes those same services as `search_job_board`, `review_job_evidence`, and `export_job_review`.
...
# Historical safety-lab documentation (synthetic workflow only)

The following describes the earlier local experiment, not the job evidence workflow above. Historical test counts and research counts are snapshots, not current measurements.
